        ...
        private IEnumerator<float> SendBroadcast(Player p)
        {
            ...
            if (Buddy.singleton.buddies.ContainsKey(p.UserId) && Buddy.singleton.Config.SendBuddyBroadcast)
            {
                ...
                else
                {
                    Player player = Player.Get(buddy1);
                    if (player == null) yield break;
                    if (Buddy.singleton.Config.SendBuddyBroadcast && !(Buddy.singleton.Config.SendBuddyHint))
                    {
                    ...
                    else if (Buddy.singleton.Config.SendBuddyHint && !(Buddy.singleton.Config.SendBuddyBroadcast))
                    {
                        p.ShowHint(Buddy.singleton.Config.GetLang("broadcastBuddy").Replace("$buddy", player.Nickname), 7.5f);
                    }
                    else if (Buddy.singleton.Config.SendBuddyBroadcast && Buddy.singleton.Config.SendBuddyHint)
                    {
                        p.Broadcast(5, Buddy.singleton.Config.GetLang("broadcastBuddy").Replace("$buddy", player.Nickname), Broadcast.BroadcastFlags.Normal);
                        p.ShowHint(Buddy.singleton.Config.GetLang("broadcastBuddy").Replace("$buddy", player.Nickname), 7.5f);
                    }
            ...
            for (int i = 0; i < hubs.Count(); i++)
            {
                ...
                if (Buddy.singleton.buddies.ContainsKey(player.UserId))
                {
                    try
                    {
                        ...
                        {
                            //SCPs take priority
                            if (buddy.Role.Team == Team.SCP) continue;
